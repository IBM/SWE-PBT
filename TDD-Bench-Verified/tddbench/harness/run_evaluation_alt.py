...
def log_baseline(instance_id, baseline_pair_prefix, baseline_test_output, baseline_timed_out, timeout, pair_log_dir, baseline_pair_suffix,
    ...
    baseline_report = {
        instance_id: {
            ...
            "cov_change_before": cov_change_before,
            "cov_miss_before": cov_miss_before
        }
    }
...
def log_f2p(instance_id, pair_prefix, test_output, timed_out, timeout, pair_log_dir, pair_suffix,
    ...
    report = {
        instance_id: {
            ...
            "resolved": resolved,
            "cov_change_after": cov_change_after,
            "cov_miss_after": cov_miss_after
        }
    }
            ...
            try:
                ...
                if not baseline_report_path.exists():
                    ...
                    if pbt_mode in ["regression", "combined"]:
                        ...
                        log_baseline(instance_id, "regression", baseline_test_output, baseline_timed_out, timeout, pair_log_dir, baseline_pair_suffix, fun2test, test_version, baseline_code_version, test_patch_value, code_patch_value, baseline_report_path)
                    ...
                    if pbt_mode in ["reproduction", "combined"]:
                        ...
                        log_baseline(instance_id, "reproduction", baseline_test_output, baseline_timed_out, timeout, pair_log_dir, baseline_pair_suffix, fun2test, test_version, baseline_code_version, test_patch_value, code_patch_value, baseline_report_path)
            ...
            for code_idx, code_patch_value in enumerate(code_patches):
                ...
                try:
                    ...
                    if pbt_mode in ["regression", "combined"]:
                        ...
                        log_f2p(instance_id, "regression", test_output, timed_out, timeout, pair_log_dir, pair_suffix, fun2test, test_version, code_version, test_patch_value, code_patch_value, report_path, baseline_pair_suffix)
                        copy_from_container(container, Path("/tmp/hypothesis_summary.json"), regression_hypothesis_summary_path, raise_on_error=False)
                    ...
                    if pbt_mode in ["reproduction", "combined"]:
                        ...
                        log_f2p(instance_id, "reproduction", test_output, timed_out, timeout, pair_log_dir, pair_suffix, fun2test, test_version, code_version, test_patch_value, code_patch_value, report_path, baseline_pair_suffix)
                        copy_from_container(container, Path("/tmp/hypothesis_summary.json"), reproduction_hypothesis_summary_path, raise_on_error=False)
...
def main(
    ...
    if keep_container == "kill":
        ...
        test_specs = list(map(make_test_spec, kill_dataset))
    ...
    if not dataset:
        ...
        run_instances(merged_predictions, dataset, cache_level, clean, force_rebuild, max_workers, log_run_id, timeout, bool(keep_container), container_run_id, pbt_mode)
