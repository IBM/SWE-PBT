...
_BUILD_RETRY_BASE_DELAY = 30  # seconds; doubles each attempt


def _is_retryable_build_error(exc: Exception) -> bool:
    """Return True when the build failure looks like a transient network problem."""
    msg = str(exc)
    return any(substr in msg for substr in _NETWORK_ERROR_SUBSTRINGS)
...
def build_image(
    ...
    for attempt in range(1, _MAX_BUILD_RETRIES + 1):
        try:
            _build_image_once(
                image_name, setup_scripts, dockerfile, platform,
                client, build_dir, nocache, attempt,
            )
            return  # success
        except BuildImageError as e:
            last_exc = e
            if attempt < _MAX_BUILD_RETRIES and _is_retryable_build_error(e):
                delay = _BUILD_RETRY_BASE_DELAY * (2 ** (attempt - 1))
                print(
                    f"[retry {attempt}/{_MAX_BUILD_RETRIES}] Transient error building "
                    f"{image_name}; retrying in {delay}s..."
                )
                time.sleep(delay)
            else:
                raise
    raise last_exc  # should be unreachable but keeps type-checkers happy
...
def _build_image_once(
    ...
    """Single attempt at building one Docker image (called by build_image with retry)."""
    # Create a logger for the build process (append on retries so history is preserved)
    log_mode = "a" if attempt > 1 else "w"
    logger = setup_logger(image_name, build_dir / "build_image.log", mode=log_mode)
    logger.info(
        f"Building image {image_name} (attempt {attempt}/{_MAX_BUILD_RETRIES})\n"
        f"Using dockerfile:\n{dockerfile}\n"
