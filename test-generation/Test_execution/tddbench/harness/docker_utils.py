...
def copy_to_container(container: Container, src: Path, dst: Path):
    ...
    # Use a unique temp file so parallel workers don't collide on the same path.
...
def copy_from_container(container: Container, src: Path, dst: Path, raise_on_error: bool = True):
    """
    ...
    Args:
        ...
        dst (Path): Destination file path on local filesystem
        raise_on_error (bool): If False, return False on error instead of raising exception
    
    Returns:
        bool: True if successful, False if file not found and raise_on_error is False
    """
    # Check if destination path is valid
    if os.path.dirname(dst) == "":
        raise ValueError(
    ...
    try:
        ...
        try:
            # Write the tar archive to the unique temp file
        ...
        # Rename the extracted file to match the destination filename.
        # The tar archive contains the file with its original name (src.name).
        extracted_file = dst.parent / src.name
        if extracted_file != dst and extracted_file.exists():
            if dst.exists():
                dst.unlink()
            extracted_file.rename(dst)
...
def exec_run_with_timeout(container, cmd, timeout: int|None=15000):
    """
...
def list_images(client: docker.DockerClient):
    ...
    # Use the low-level API to avoid inspect_image calls that crash on
    # dangling/corrupt image layers (404 from Docker daemon).
    tags = set()
    for raw in client.api.images(all=True):
        try:
            tags.update(raw.get("RepoTags") or [])
        except Exception as e:
            print(f"[list_images] Skipping unreadable image entry: {e}")
    return tags
...
def should_remove(
    ...
    # Hard override: keep existing images for now to speedup
    return False
