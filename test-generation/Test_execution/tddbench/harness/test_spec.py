...
class TestSpec:
    ...
    def eval_script(self):
        return "\n".join(["#!/bin/bash", "set -uo pipefail"] + self.eval_script_list) + "\n"
...
def make_env_script_list(instance, specs, env_name):
    ...
    # Install hypothesis and pytest for property-based testing
    # Installing during environment setup ensures they're available and avoids DNS issues at runtime
    # Pin hypothesis to a known-good range: >=6 for --hypothesis-log-all support,
    # but below the versions that pull in sparse/numba (which conflict with coverage<5).
...
def make_eval_script_list(instance, specs, env_name, repo_directory, base_commit, test_patch):
    ...
    # Slight modification to run standalone pbt test script
    # test_command = " ".join(
    #     [
    ...
    #     special_tox1='export PYTEST_ADDOPTS="--cov=sphinx --cov-branch --cov-report=term-missing"'

    #     coverage_install="python3 -m pip install coverage"
    #     coverage_command="echo \"+ coverage report\" ; coverage report --show-missing --branch --include='/testbed/sphinx/*'"
    ...
    # Overriding the Sphinx case because all PBTs are run through PyTest. So we don't have to run Sphinx PBT through tox.
    if True:
