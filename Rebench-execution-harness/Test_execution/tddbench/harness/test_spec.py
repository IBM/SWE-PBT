...
def make_eval_script_list(instance, specs, env_name, repo_directory, base_commit, test_patch):
    ...
    # Get test command with default fallback for unknown repos/versions
    repo = instance["repo"]
    version = instance["version"]
    
    # Use MAP_REPO_VERSION_TO_SPECS or default pytest command with coverage
    # (Don't use test_cmd from install_config)
    if repo in MAP_REPO_VERSION_TO_SPECS and version in MAP_REPO_VERSION_TO_SPECS[repo]:
        test_cmd = MAP_REPO_VERSION_TO_SPECS[repo][version]["test_cmd"]
    else:
        # Default test command for unknown repos/versions
        test_cmd = "coverage run --branch -m pytest -rA"
    ...
    if instance["repo"].strip()=="sphinx-doc/sphinx":
        ...
        coverage_install="python3 -m pip install coverage pytest"
...
def make_test_spec(instance: SWEbenchInstance) -> TestSpec:
    ...
    # Handle missing repo or version with default pytest-based spec
    elif repo not in MAP_REPO_VERSION_TO_SPECS:
        # Default spec for unknown repositories
        specs = {
            "python": "3.10",
        ...
        # Default spec for unknown versions
        specs = {
            "python": "3.10",
            "install": "python -m pip install -e .",
            "test_cmd": "pytest -xvs",
