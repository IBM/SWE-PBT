...
import traceback
import docker
from docker.errors import BuildError as DockerBuildError, ImageNotFound, NotFound
        ...
        for chunk in response:
            if "stream" in chunk:
                ...
                raise DockerBuildError(
                    chunk["errorDetail"]["message"], build_log=iter([{"stream": buildlog}])
                )
...
def get_env_configs_to_build(
    ...
    for test_spec in test_specs:
        # Benchmark-provided images are treated as prebuilt environment images.
        # If they already exist locally, skip any build checks entirely.
        if test_spec.benchmark_image:
            try:
...
def build_env_images(
    ...
    if force_rebuild:
        ...
        for key in env_image_keys:
            remove_image(client, key, "quiet")
    if any(not spec.benchmark_image for spec in test_specs):
    ...
    # Build or pull the environment images
    successful, failed = list(), list()
    with tqdm(
        total=len(configs_to_build), smoothing=0, desc="Preparing environment images"
    ) as pbar:
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = {}
            for image_name, config in configs_to_build.items():
                if config.get("pull_only"):
                    # Pull benchmark images without platform specification to avoid mismatch
                    futures[executor.submit(client.images.pull, image_name)] = image_name
                else:
                    futures[
                        executor.submit(
                            build_image,
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    print(f"Error preparing image {futures[future]}")
                    traceback.print_exc()
                    failed.append(futures[future])
                    continue
    ...
    if len(failed) == 0:
        print("All environment images prepared successfully.")
    else:
        print(f"{len(failed)} environment images failed to prepare.")
...
def build_instance_image(
    ...
    try:
        ...
        action = "pull" if test_spec.benchmark_image else "build"
        logger.info(
            f"Environment image {env_image_name} not found for {test_spec.instance_id}. "
            f"Attempting to {action} it now..."
        )
        try:
            build_env_images(client, [test_spec], force_rebuild=False, max_workers=1)
            env_image = client.images.get(env_image_name)
            logger.info(f"Successfully prepared environment image {env_image_name}")
        except Exception as build_error:
            raise BuildImageError(
                test_spec.instance_id,
                f"Failed to prepare environment image {env_image_name} for {test_spec.instance_id}: {str(build_error)}",
                logger,
            ) from build_error
    logger.info(
    ...
    # If benchmark_image is provided, use it directly as the instance image (skip building)
    if test_spec.benchmark_image:
        logger.info(
            f"Using benchmark image {env_image_name} directly as instance image for {test_spec.instance_id}. "
            f"Skipping instance image build."
        )
        # Tag the benchmark image with the instance image name if they're different
        if env_image_name != image_name:
            try:
                env_image.tag(image_name)
                logger.info(f"Tagged {env_image_name} as {image_name}")
            except Exception as e:
...
def build_container(
    ...
    try:
        ...
        # Use default config if repo not found in MAP_REPO_VERSION_TO_SPECS
        if test_spec.repo in MAP_REPO_VERSION_TO_SPECS:
            config = MAP_REPO_VERSION_TO_SPECS[test_spec.repo].get(test_spec.version, {})
        else:
            # Default configuration for unmapped repositories
            config = {}
            logger.warning(f"Repository {test_spec.repo} not found in MAP_REPO_VERSION_TO_SPECS, using default config")
        
        user = "root" if not config.get("execute_test_as_nonroot", False) else "root"
        nano_cpus = config.get("nano_cpus")

        # Create the container
        logger.info(f"Creating container for {test_spec.instance_id}...")
        # Don't specify platform for benchmark images to avoid mismatch errors
        container_kwargs = {
        ...
        if not test_spec.benchmark_image:
            # Only specify platform for non-benchmark images
            container_kwargs["platform"] = test_spec.platform
