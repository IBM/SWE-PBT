...
from tddbench.harness.log_parsers import get_parser_for_repo
...
def get_logs_eval(log_fp: str) -> tuple[dict[str, str], bool]:
    ...
    # Use pytest_v2 parser as default if repo not found in MAP_REPO_TO_PARSER
    # pytest_v2 supports both old format (status at start) and new format (status at end)
    log_parser = get_parser_for_repo(repo)
