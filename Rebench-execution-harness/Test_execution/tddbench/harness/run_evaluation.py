...
from tddbench.harness.test_spec import make_eval_script_list, make_test_spec, TestSpec
...
def run_instance(
    ...
    model_name_or_path = "mutation_model"  # Use constant name instead of dynamic path
    ...
    try:
        ...
        for test_idx, test_patch_value in enumerate(test_patches):
            ...
            reset_command = "git reset --hard && git clean -fd && find /testbed -name '*.rej' -delete && find /testbed -name '*.orig' -delete"
            ...
            for test_file in contributing_functions:
                ...
                test_output, _, _ = exec_run_with_timeout(container, command, timeout)
            ...
            # Handle missing repo or version with default pytest-based spec
            if test_spec.repo not in MAP_REPO_VERSION_TO_SPECS:
                specs = {
                    "python": "3.10",
                    "install": "python -m pip install -e .",
                    "test_cmd": "pytest -xvs",
                }
            elif test_spec.version not in MAP_REPO_VERSION_TO_SPECS[test_spec.repo]:
                specs = {
                    "python": "3.10",
                    "install": "python -m pip install -e .",
                    "test_cmd": "pytest -xvs",
            ...
            test_version_spec.eval_script_list = make_eval_script_list(
                instance_payload,
                specs,
                env_name,
                repo_directory,
            ...
            try:
                ...
                if not baseline_report_path.exists():
                    ...
                    baseline_test_output, baseline_timed_out, baseline_total_runtime = exec_run_with_timeout(container, f"/bin/bash {container_test_eval_path}", timeout)
                    ...
                    baseline_combined_output_path = pair_log_dir / f"test_coverage_combine{baseline_pair_suffix}.txt"
                    ...
                    baseline_test_output_path = pair_log_dir / f"test_output{baseline_pair_suffix}.txt"
                    baseline_coverage_output_path = pair_log_dir / f"test_coverage{baseline_pair_suffix}.txt"
                    ...
                    baseline_test_result, _ = get_logs_eval(str(baseline_test_output_path))
            ...
            for code_idx, code_patch_value in enumerate(code_patches):
                ...
                try:
                    ...
                    test_output, timed_out, total_runtime = exec_run_with_timeout(container, f"/bin/bash {container_test_eval_path}", timeout)
                    ...
                    combined_output_path = pair_log_dir / f"test_coverage_combine{pair_suffix}.txt"
                    ...
                    test_output_path = pair_log_dir / f"test_output{pair_suffix}.txt"
                    coverage_output_path = pair_log_dir / f"test_coverage{pair_suffix}.txt"
                    with open(test_output_path, "w") as f:
                        f.write(test_after)
                    with open(coverage_output_path, "w") as f:
                        f.write(coverage_after)

                    test_result, _ = get_logs_eval(str(test_output_path))
...
def make_run_report(
    ...
    report_file = Path(
        "mutation_model"  # Use constant name instead of dynamic path
        + f".{run_id.replace('/', '_')}"
        + ".json"
    )
...
def main(
    ...
    if not dataset:
        ...
        run_instances(merged_predictions, dataset, cache_level, clean, force_rebuild, max_workers, log_run_id, timeout, keep_container, container_run_id)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str2bool,
        default=False,
        help="Keep containers after evaluation (allows reuse in subsequent runs)"
    )
    parser.add_argument(
        "--output_subdir",
        type=str,
