...
def get_parser_for_repo(repo: str, parser_name: str | None = None):
    """
    Get the appropriate log parser for a repository.
    Falls back to parse_log_pytest_v2 for unknown repositories.
    
    Args:
        repo (str): Repository name in format "owner/repo"
        parser_name (str, optional): Name of the parser function to use (e.g., "parse_log_pytest")
    
    Returns:
        callable: Parser function for the repository
    """
    # If parser_name is provided, try to get it from globals
    if parser_name:
        # Handle both "parse_log_pytest" and "pytest" formats
        if not parser_name.startswith("parse_log_"):
            parser_name = f"parse_log_{parser_name}"
        parser_func = globals().get(parser_name)
        if parser_func and callable(parser_func):
            return parser_func
    
    # Fall back to repo-based lookup or default
    return MAP_REPO_TO_PARSER.get(repo, parse_log_pytest_v2)
